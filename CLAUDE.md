...
`file_path`, `topic`, or `limit` to narrow. Returns every graduated
prompt (≥7 by /coach gate) with its actual `graduated_overall_score` and
`reuse_count`, ranked by score then reuse. Differs from `wiki_lookup`,
which is path-anchored — this is the FULL library for discovery.
